...
function _setupKeymapSearchUI(groupbox) {
	...
	function applyFilters() {
		...
		groupHeadings.forEach((h2) => {
			...
			const anyVisible = [...groupbox.querySelectorAll(`.zenCKSOption-input[data-group="${groupId}"]`)].some(
...
function addSettingKeymapSearch() {
	...
	const checkAndSetup = () => {
		...
		const hasValidHeadings = Array.from(groupHeadings).some((h2) => h2.textContent.trim().length > 0)
...
const observer = new MutationObserver((mutations) => {
	...
	for (const mutation of mutations) {
		if (mutation.type === 'childList') {
			...
			if (
				Array.from(mutation.addedNodes).some((node) => node.nodeType === Node.ELEMENT_NODE && node.id === 'zenCKSGroup')
			) {
			...
			if (
				Array.from(mutation.addedNodes).some(
					(node) => node.nodeType === Node.ELEMENT_NODE && node.querySelector && node.querySelector('#zenCKSGroup')
				)
			) {
				groupboxAppeared = true
				break
