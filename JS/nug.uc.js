...
if (!window.urlBarModifierInitialized) {
	...
	const setupCSSPropertyObserver = () => {
		...
		let lastKnownColor = getComputedStyle(document.documentElement).getPropertyValue('--zen-primary-color').trim()
		...
		function checkColorChange() {
			const currentColor = getComputedStyle(document.documentElement).getPropertyValue('--zen-primary-color').trim()
	...
	const onWorkspaceColorChange = (newColor) => {
		...
		window.dispatchEvent(
			new CustomEvent('zen-workspace-color-changed', {
		...
		mutations.forEach((mutation) => {
			...
			if (mutation.type === 'attributes') {
				...
				if (target.id === 'unified-extensions-panel' || target.id === 'unified-extensions-view') {
					...
					if (
						target.id === 'unified-extensions-panel' &&
						(mutation.attributeName === 'hidden' || mutation.attributeName === 'style')
					) {
						const isHidden =
							target.hasAttribute('hidden') || target.style.display === 'none' || target.style.visibility === 'hidden'
						if (!isHidden) {
